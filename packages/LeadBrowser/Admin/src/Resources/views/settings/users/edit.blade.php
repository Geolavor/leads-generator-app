...
@section('content-wrapper')
    <div class="content full-page adjacent-center">
        {!! view_render_event('settings.users.edit.header.before', ['admin' => $admin]) !!}
        ...
        {!! view_render_event('settings.users.edit.header.after', ['admin' => $admin]) !!}

        <form method="POST" action="{{ route('settings.users.update', ['id' => $admin->id]) }}" @submit.prevent="onSubmit">
            <div class="page-content">
                <div class="form-container">
                    <div class="panel">
                        <div class="panel-header">
                            {!! view_render_event('settings.users.edit.form_buttons.before', ['admin' => $admin]) !!}
                            ...
                            {!! view_render_event('settings.users.edit.form_buttons.after', ['admin' => $admin]) !!}
                        </div>

                        <tabs>
                            <tab name="{{ __('admin::app.settings.users.general') }}" :selected="true">
                                {!! view_render_event('settings.users.edit.form_controls.general.before', ['admin' => $admin]) !!}
                                ...
                                {!! view_render_event('settings.users.edit.form_controls.general.after', ['admin' => $admin]) !!}
                            </tab>

                            <tab name="{{ __('admin::app.settings.users.permission') }}">
                                {!! view_render_event('settings.users.edit.form_controls.permission.before', ['admin' => $admin]) !!}
                                ...
                                {!! view_render_event('settings.users.edit.form_controls.permission.after', ['admin' => $admin]) !!}
                            </tab>

                            <tab name="{{ __('admin::app.settings.users.payments') }}">
                                {!! view_render_event('settings.users.edit.form_controls.payments.before', ['admin' => $admin]) !!}

                                @csrf()
                                
                                <input name="_method" type="hidden" value="PUT">
                                
                                <div class="form-group" :class="[errors.has('wallet') ? 'has-error' : '']">
                                    <label class="required">
                                        {{ __('admin::app.settings.users.wallet') }}
                                    </label>
                                    ...
                                    <input
                                        ...
                                        name="wallet"
                                        class="control"
                                        value="{{ old('wallet') ?? $admin->balance() }}"
                                        placeholder="{{ __('admin::app.settings.users.wallet') }}"
                                        v-validate="'required'"
                                        data-vv-as="{{ __('admin::app.settings.users.wallet') }}"
                                    />

                                    <span class="control-error" v-if="errors.has('wallet')">
                                        @{{ errors.first('wallet') }}
                                    </span>
                                </div>

                                {!! view_render_event('settings.users.edit.form_controls.payments.after', ['admin' => $admin]) !!}
                            </tab>
                        </tabs>
                    </div>
                </div>
