...
@section('content-wrapper')
    <div class="content full-page">
        <table-component data-src="{{ route('settings.tags.index') }}">
            <template v-slot:table-header>
                <h1>
                    {!! view_render_event('settings.tags.index.header.before') !!}

                    {{ Breadcrumbs::render('settings.tags') }}

                    {{ __('admin::app.settings.tags.title') }}

                    {!! view_render_event('settings.tags.index.header.after') !!}
                </h1>
            </template>

            @if (bouncer()->hasPermission('settings.other_settings.tags.create'))
                <template v-slot:table-action>
                    <button class="btn btn-primary" @click="openModal('addTagModal')">{{ __('admin::app.settings.tags.create-title') }}</button>
                </template>
            @endif
        <table-component>
    </div>

    <form action="{{ route('settings.tags.store') }}" method="POST" @submit.prevent="onSubmit">
        <modal id="addTagModal" :is-open="modalIds.addTagModal">
            <h3 slot="header-title">{{ __('admin::app.settings.tags.create-title') }}</h3>

            <div slot="header-actions">
                {!! view_render_event('settings.tags.create.form_buttons.before') !!}
                ...
                {!! view_render_event('settings.tags.create.form_buttons.after') !!}
            </div>

            <div slot="body">
                {!! view_render_event('settings.tags.create.form_controls.before') !!}
                ...
                {!! view_render_event('settings.tags.create.form_controls.after') !!}
            </div>
        </modal>
    </form>
@stop