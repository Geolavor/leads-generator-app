...
@section('content-wrapper')
    <div class="content full-page">
        <form method="POST" action="{{ route('sales.refunds.store', $order->id) }}" @submit.prevent="onSubmit">
            @csrf()
                    ...
                    <table>
                        ...
                        <tbody>
                            @foreach ($order->items as $item)
                                <tr>
                                    <td>{{ LeadBrowser\Product\Helpers\ProductType::hasVariants($item->type) ? $item->child->sku : $item->sku }}</td>
        ...
        Vue.component('refund-items', {
            ...
            methods: {
                updateQty: function() {
                    ...
                    this.$http.post("{{ route('sales.refunds.update_qty', $order->id) }}", this.refund.items)
