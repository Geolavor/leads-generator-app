...
use LeadBrowser\Sales\Generators\OrderSequencer;
use LeadBrowser\Sales\Models\Order as OrderModel;
use LeadBrowser\Sales\Models\OrderPayment;
use LeadBrowser\Sales\Models\OrderPaymentProxy;
...
class OrderRepository extends Repository
{
     ...
     *
     * @return \LeadBrowser\Sales\Contracts\Order
     */
    public function createOrderIfNotThenRetry(array $data)
    {
        ...
        try {
            ...
            $ord = new OrderPayment();
            $ord->order_id = $order->id;
            $ord->method = 'Test';
            $ord->method_title = 'Test';
            $ord->save();

            unset($data['billing_address']['user_id']);
     ...
     *
     * @param  \LeadBrowser\Sales\Models\Order|int  $orderOrId
     * @return \LeadBrowser\Sales\Contracts\Order
     */
    ...
    {
        $totalQtyOrdered = $totalQtyInvoiced = $totalQtyRefunded = $totalQtyCanceled = 0;
    ...
    public function collectTotals($order)
    {
        ...
        $order->grand_total_invoiced = $order->sub_total_invoiced + $order->tax_amount_invoiced - $order->discount_invoiced;
        $order->base_grand_total_invoiced = $order->base_sub_total_invoiced  + $order->base_tax_amount_invoiced - $order->base_discount_invoiced;
        ...
        $order->grand_total_refunded += $order->sub_total_refunded  + $order->tax_amount_refunded - $order->discount_refunded;
        $order->base_grand_total_refunded += $order->base_sub_total_refunded + $order->base_tax_amount_refunded - $order->base_discount_refunded;
