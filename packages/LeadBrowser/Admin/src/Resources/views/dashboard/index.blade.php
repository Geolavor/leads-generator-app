...
@section('content-wrapper')
    ...
    <div class="content full-page dashboard">
        ...
        {!! view_render_event('dashboard.index.filter.before') !!}
        ...
        {!! view_render_event('dashboard.index.cards.after') !!}

        @if($first_time_login)
            <welcome-modal :show="{{ $first_time_login }}"></welcome-modal>
        @endif
    </div>
@stop
...
@push('scripts')
    ...
    <script type="text/x-template" id="card-template">
        ...
        <div v-else :class="`card-data ${className || ''}`">
            ...
            <div class="search" v-if="cardType == 'top_search_card'" v-for="(data, index) in dataCollection.data">
                <a :href="'{{ route('search.location.view') }}/' + data.id">@{{ data.title }}</a>
            ...
            <div class="lead" v-else-if="cardType == 'top_card'" v-for="(data, index) in dataCollection.data">
                ...
                @if (bouncer()->hasPermission('leads.view')) 
                    <a :href="'{{ route('leads.view') }}/' + data.id">@{{ data.title }}</a>
                @else
                    @{{ data.title }}</a>
                @endif
            ...
            <template v-if="! dataCollection || dataCollection.length == 0 || (dataCollection.data && dataCollection.data.length == 0)">
                <div class="custom-card">
                    ...
                    <img
                        src="{{ asset('vendor/leadBrowser/admin/assets/images/empty-state-icon.svg') }}"
                    />
        ...
        Vue.component('selected-cards-filter', {
            ...
            methods: {
                updateCards: function (dateRange) {
                    this.$http.post(`{{ route('api.dashboard.cards.update') }}`, {
                        cards: this.columns
                    }).then(response => {
        ...
        Vue.component('cards-collection', {
            ...
            methods: {
                getDashboardCards: function () {
                    this.$http.get(`{{ route('api.dashboard.cards.index') }}`)
                        .then(response => {
                            this.cards = response.data;
        ...
        Vue.component('card-component', {
            ...
            methods: {
                getCardData: function (cardId, filter, filterKey) {
                    ...
                    this.$http.get(`{{ route('api.dashboard.card.index') }}?card-id=${cardId}${filter ? `&${filterKey}=${filter}` : ''}`)
                        .then(response => {
