    ...
    <script type="text/x-template" id="kanban-component-tempalte">
        <kanban-board :stages="stage_names" :blocks="blocks" @update-block="updateLeadStage">
            <div v-for="(stage, index) in stage_names" :slot="stage" :key="`stage-${stage}`">
                ...
                @if (bouncer()->hasPermission('crm.leads.create'))
                    <a :href="'{{ route('leads.create') }}' + '?stage_id=' + stages[index].id">
            ...
            >
                ...
                <div class="icons">
                    <a :href="'{{ route('leads.view') }}/' + block.id" class="icon eye-icon"></a>
                ...
                <div class="lead-cost">
                    <i class="icon bi-wallet2"></i>@{{ block.lead_value }}
                </div>
            </div>
        </kanban-board>
    </script>
        ...
        Vue.component('kanban-component', {
            ...
            methods: {
                getLeads: function (searchedKeyword, filterValues) {
                    ...
                    this.$http.get("{{ route('leads.get', request('pipeline_id')) }}" + `${searchedKeyword ? `?search=${searchedKeyword}` : ''}${filterValues || ''}`)
                        .then(response => {
                ...
                updateLeadStage: function (id, stageName) {
                    ...
                    this.$http.put("{{ route('leads.update') }}/" + id, {'lead_pipeline_stage_id': stage[0].id})
                        .then(response => {
                            this.getLeads();
