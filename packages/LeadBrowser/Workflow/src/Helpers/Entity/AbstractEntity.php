...
abstract class AbstractEntity
{
     ...
     * @return array
     */
    public function getAttributes($entityType, $skipAttributes = ['textarea', 'image', 'file', 'address', 'location'])
    {
        $attributes = [];

        foreach ($this->attributeRepository->findByField('entity_type', $entityType) as $attribute) {
            
            if (in_array($attribute->type, $skipAttributes) || $attribute->is_visible != 2) {
                continue;
            }
        ...
        foreach ($this->getAttributes($this->entityType, []) as $attribute) {
            ...
            switch ($attribute['type']) {
                ...
                case 'location':
                    ...
                    $value = core()->state_name($entity->{$attribute['id']}['state']) . "<br>" . core()->country_name($entity->{$attribute['id']}['country']) . "<br>";
