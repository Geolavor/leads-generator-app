...
use LeadBrowser\Admin\DataGrids\InvoicesTransactionsDatagrid;
use LeadBrowser\Admin\DataGrids\Order\OrderInvoicesDataGrid;
use LeadBrowser\Admin\Http\Controllers\Controller;
use LeadBrowser\Admin\Traits\Mails;
use LeadBrowser\Core\Traits\PDFHandler;
...
class InvoiceController extends Controller
{
    ...
    public function store($orderId)
    {
        ...
        if (! $order->canInvoice()) {
            session()->flash('warning', trans('admin::app.sales.invoices.creation-error'));
        ...
        if (! $this->invoiceRepository->haveProductToInvoice($data)) {
            session()->flash('warning', trans('admin::app.sales.invoices.product-error'));

            return redirect()->back();
        }

        if (! $this->invoiceRepository->isValidQuantity($data)) {
            session()->flash('warning', trans('admin::app.sales.invoices.invalid-qty'));
    ...
    public function sendDuplicateInvoice(Request $request, $id)
    {
        ...
        session()->flash('warning', __('admin::app.response.something-went-wrong'));
