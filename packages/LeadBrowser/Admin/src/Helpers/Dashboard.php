...
namespace LeadBrowser\Admin\Helpers;
...
use LeadBrowser\Lead\Repositories\ProductRepository as LeadProductRepository;
...
class Dashboard
{
     ...
     *
     * @var \LeadBrowser\Lead\Repositories\PipelineRepository
     */
     ...
     * @param  \LeadBrowser\Lead\Repositories\PipelineRepository  $pipelineRepository
    ...
    /**
     * Collect search card data.
     *
     * @return array|boolean
     */
    public function getSearches($startDateFilter, $endDateFilter, $totalWeeks)
    {
     ...
     * @return array
     */
    public function getMailboxs($startDateFilter, $endDateFilter, $totalWeeks)
    {
        $totalMailboxs = $receivedMailboxs = $draftMailboxs = $outboxMailboxs = $sentMailboxs = $trashMailboxs = 0;

        $mailboxsCollection = $this->mailboxRepository
            ->whereBetween('created_at', [$startDateFilter, $endDateFilter])
            ->get();

        foreach ($mailboxsCollection as $key => $mailbox) {
            if (in_array('inbox', $mailbox->folders) !== false) {
                $receivedMailboxs++;
            } else if (in_array('draft', $mailbox->folders) !== false) {
                $draftMailboxs++;
            } else if (in_array('outbox', $mailbox->folders) !== false) {
                $outboxMailboxs++;
            } else if (in_array('sent', $mailbox->folders) !== false) {
                $sentMailboxs++;
            } else if (in_array('trash', $mailbox->folders) !== false) {
                $trashMailboxs++;
            }
