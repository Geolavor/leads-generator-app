<div class="content full-page">
    <div class="table">
        <div class="table-header">
            <h1>
                {!! view_render_event('activities.index.header.before') !!}

                {{ Breadcrumbs::render('activities') }}

                {{ __('admin::app.activities.title') }}

                {!! view_render_event('activities.index.header.after') !!}
            </h1>
        </div>
        ...
        Vue.component('calendar-component', {
            ...
            methods: {
                getActivities: function ({startDate, endDate}) {
                    ...
                    this.$http.get("{{ route('activities.get', ['view_type' => 'calendar']) }}" + `&startDate=${new Date(startDate).toLocaleDateString("en-US")}&endDate=${new Date(endDate).toLocaleDateString("en-US")}`)
                        .then(response => {
                ...
                onEventClick : function (event) {
                    window.location.href = "{{ route('activities.edit') }}/" + event.id
                }
            }
        });
    </script>
